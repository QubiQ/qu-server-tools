...
class ImportPartnerSupplier(models.TransientModel):
    ...
    def _update_values(self, values):
        ...
        if values['category_id']:
            category_id = self.env['res.partner.category'].search([
                ('name', 'in', values['category_id'].split(','))
            ])
            if category_id:
                values['category_id'] = [(6, 0, category_id.ids)]
            else:
                category_ids = []
    ...
    def _assign_partner_data(self, values):
        ...
        values['company_id'] = int(values.get('company_id')) if values.get('company_id') else 1
    ...
    def _create_new_partner(self, values, props):
        ...
        if bank_info:
            ...
            bank_obj = self.sudo().env['res.partner.bank'].create(bank_info)
